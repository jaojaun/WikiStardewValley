...
export interface CadItemScreenProps {
    navigation:any 
}

export const CadItemScreen   = (setUser: any) => {
    ...
    const [ item, setItem ] = useState<Item>({nome: '', descricao: ''});
    useEffect(() => {
        if (auth.currentUser) {getDoc(doc(db, 'usuarios', auth.currentUser.uid))
                .then(dados => setItem(dados.data()))
        }
    }, []);

    const Handlecadastrar = async ({nome, descricao, local}: any) => {
